...
async function start() {
    ...
    if (options.init || options.keys) {
        ...
        const result = DeckXstreamConfig.safeParse((await import(options.config, {with: {type: 'json'}})).default);
        if (result.success) 
            config = result.data;
        else {
    ...
    const buttons = Array.from({ length: streamDeck.CONTROLS.filter((ctl) => ctl.type === 'button').length });
...
function getDevice(devices: StreamDeckDeviceInfo[], serialPathOrModel?: string) {
    const result = devices.find((dev) => {
        return serialPathOrModel && (dev.serialNumber === serialPathOrModel || dev.path === serialPathOrModel || dev.model === serialPathOrModel);
    });
