...
const PIXEL2 =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQYV2NI0i77DwAD+QID4vYuBwAAAABJRU5ErkJggg=='
...
describe('Buttons', function () {
  ...
  describe('Static buttons', function () {
    ...
    it('should allow changing context for text', async function () {
      ...
      let bc2 = new buttonController(dm, {
        keyIndex: 1,
        icon: PIXEL,
        text: 'A',
        textSettings: { fillStyle: 'blue' },
      })
      await bc.init()
      await bc2.init()
      expect(bc.pages[0].buffer).to.not.eql(bc2.pages[0].buffer)
    ...
    it('should process each frame of a gif', async function () {
      bc = new buttonController(
        ...
        {
          keyIndex: 0,
          icon: require('path').join(__dirname, '../test_resources/blink.gif'),
        },
      )
