...
async function start() {
	...
	const buttons = Array.from({ length: streamDeck.device.deviceProperties.CONTROLS.filter((ctl) => ctl.type === 'button').length });

	if (options.keys || options.init) {
		config = {
			"deckxstream-version": 2,
			"brightness": 90,
...
function getDevice(devices, str) {
